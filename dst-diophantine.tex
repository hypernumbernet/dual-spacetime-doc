...
The finiteness of the unit group of the discrete biquaternion ring implies that every power-sum Diophantine equation
\[
\sum_i c_i\, b_i^{e_i} = 0
\]
...
\begin{proof}
Immediate from \(J=\frac12\sum_a(\alpha_a^2-\beta_a^2)\) and Definition~\ref{def:Jnorm}. The identity is machine-checked in the Lean~4 module \texttt{DstDiophantine.Framework.Descent}.
\end{proof}
...
\end{itemize}
All proofs rest on the same mechanism: the non-existence of configurations that simultaneously satisfy the algebraic relation and violate the bound \( |\Jnorm| \leq 1 \), together with the finiteness of the admissible discrete search space (Chapter~10). Dual extraction by dagger preserves \(|\Jnorm|\) and is not itself a height-reducing descent.
