...
\[
J_{\rm Beal} \geq m^2 \varepsilon > 1
\]
...
\]
Proposition~\ref{prop:quality-torsion} forces \( H_{\rm abc} > 1 \), contradicting the torsion boundedness theorem (Chapter~3).
...
\begin{proof}
Suppose, for contradiction, that a primitive triple satisfies \( q(a,b,c) > Q(N) \). By Proposition~\ref{prop:quality-torsion} the abc torsional height obeys \( H_{\rm abc}(a,b,c) > 1 \). Because \( \Omega_{\rm abc} \) is a physically admissible configuration on the discrete torus, the torsion boundedness theorem (Chapter~3) requires \( |J| \leq 1 \) for all components of the mismatch. The height \( H_{\rm abc} \) is precisely the quadratic form \( J \) evaluated on \( \Omega_{\rm abc} \), so \( H_{\rm abc} > 1 \) is a contradiction.
...
The continuum recovery is uniform: the discrete proof for each fixed \( N \) supplies a finite verification set, and the limit \( N \to \infty \) eliminates the artificial ceiling while preserving the bound \( |J| \leq 1 \) that drives the contradiction.
...
\label{cor:abc-beal}
When \( A^x + B^y = C^z \) with \( \gcd(A,B,C) = 1 \) and \( x,y,z \geq 3 \), the abc bound on the underlying additive triple \( A^x + B^y = C^z \) forces \( H_{\rm abc} > 1 \) for large exponents, consistent with the coprimality contradiction of Chapter~6. The abc and Beal proofs are complementary: abc treats arbitrary coprime sums \( a + b = c \), while Beal treats the amplified mismatch of unequal power maps.
\end{corollary}
...
\]
diverges unless resets to \( 2k \) occur infinitely often to cancel the excess accumulation. On the discrete torus the mismatch coefficients are integer multiples of \( 2\pi/N \); any configuration with \( J(\Sigma_M) > 1 \) is forbidden. Lemma~\ref{lem:gap-accumulation} below makes the discrete threshold explicit.
\end{proof}

\begin{proposition}[Reset characterization]
\label{prop:gap-reset}
Fix an even gap \( 2k \). On the discrete torus, the total chain mismatch satisfies \( |J(\Sigma_M)| \leq 1 \) for all \( M \) if and only if the prime gap sequence resets to \( 2k \) infinitely often. Each reset at index \( n \) corresponds to a Polignac pair \( (p_n, p_{n+1}) \).
\end{proposition}

\begin{proof}
A reset at \( n \) gives \( g_n = 2k \), so the corresponding summand \( (g_n/p_n)\, i\Gamma_a \) is the exact cancellation increment for the gap mismatch rotor at \( p_n \). By Definition~\ref{def:gap-mismatch}, \( J(\Omega_{2k}(p_n)) = 0 \) at each reset. If resets occurred only finitely often, Proposition~\ref{prop:gap-avoidance} would force \( J(\Sigma_M) > 1 \) for sufficiently large \( M \), contradicting the torsion boundedness theorem. Conversely, if resets to \( 2k \) occur infinitely often, each reset removes the excess accumulated by subsequent larger gaps, keeping the partial sums within the compact range enforced by the finite unit group and the principal branch bound \( |\delta_a| \leq \pi/2 \) (Lemma on Principal Branch Bound, Appendix).
\end{proof}
...
\begin{proof}
Suppose, for contradiction, that \( g_n \neq 2k \) for all sufficiently large \( n \). Then there exists \( N_0 \) as in Proposition~\ref{prop:gap-avoidance}, and the cumulative mismatch satisfies \( J(\Sigma_M) > 1 \) for all sufficiently large \( M \). Because \( \Sigma_M \) is a physically admissible configuration on the discrete torus, the torsion boundedness theorem (Chapter~3) requires \( |J(\Sigma_M)| \leq 1 \), a contradiction.
...
The continuum recovery is uniform: the discrete proof for each fixed \( N \) supplies a finite verification set for the initial segment of the chain, while the limit \( N \to \infty \) eliminates the artificial truncation and preserves the bound \( |J| \leq 1 \) that drives the contradiction.
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item the associated mismatch satisfies \(|J(R_N(A))| \leq 1\).
\end{enumerate}
\end{definition}
...
\begin{lemma}
For fixed compactification parameter \( N \) there exists a constant \( Q(N) > 1 \) such that every primitive triple \( (a,b,c) \) with \( a + b = c \) and \( q(a,b,c) > Q(N) \) satisfies \( J(\Omega_{\rm abc}) > 1 \), where \( \Omega_{\rm abc} \) is the abc mismatch rotor of Chapter~7.
\end{lemma}

\begin{proof}
By Proposition~\ref{prop:quality-torsion}, large quality forces the abc torsional height \( H_{\rm abc} = J(\Omega_{\rm abc}) \) to exceed \( c_1(N)(q - 1)^2 \). Choose \( Q(N) = 1 + (c_1(N) \varepsilon_N)^{-1/2} \) where \( \varepsilon_N \) is the minimal nonzero torsional height on the discrete torus. Then \( q(a,b,c) > Q(N) \) implies \( H_{\rm abc} > c_1(N) \varepsilon_N \geq 1 \), using the principal branch bound and the compactness of the dual sector from Lemma~\ref{lem:radical-dual}.
\end{proof}
