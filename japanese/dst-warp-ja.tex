...
この枠組みにおいて、\textbf{ワープ推進}は計量工学ではなく \emph{共鳴チューニング}である：集団的双対ローター位相をシフトして反発的ねじれ層（$J<0$）に進入し、実効変位が局所光速限界を超える一方で、情報は光チャネルに拘束される。本稿は PGA--DST の恒星間推進への \emph{仮説的}拡張である。以前の DST ワープ解析の定量的エネルギー推定を保ちつつ、PGA 代数構造、反ねじれ場（AT-field）生成~\cite{atfield}、PGA 教科書プログラムからの重力工学機構に根拠を置く。
...
\begin{definition}[反ねじれ場~\cite{atfield}]
反ねじれ場（AT-field）は $J(x)<0$ なる領域 $\mathcal{R}$ である。双対セクター回転が支配し、実効的反発をもたらす。AT-field は原理的に $\theta_a$ のコヒーレント励起により工学的に生成可能である。
\end{definition}

ワープ進入において、AT-field は単なる障壁ではなく \emph{運用媒体}である：船体は局所生成または宇宙論的に遭遇した $J<0$ 殻へ遷移し、重力結合を低減したコーストを行い、所定の位相境界で退出する。AT-field の摂動下での安定性は、自己強化型不整合ダイナミクス~\cite{atfield}から従う：外部物質の侵入は $|\beta_a|$ を増大させ、$J$ をより負に駆動する。
...
\end{equation}
同一 THz 空洞による能動フィードバックが不要励起を減衰させる。AT-field 自己強化~\cite{atfield} は外部摂動に対する追加安定性を提供する。
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item \textbf{AT-field 生成。} 高強度円偏光 THz（$10^{12}$--$10^{15}\,\mathrm{Hz}$）で局所的に $J<0$ を駆動；反発偏向または重量低減を測定。
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item 反発層（$J<0$）と AT-field が実効超光速コーストの運用媒体を提供する。
