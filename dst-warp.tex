...
\section{Repulsive Layers and Anti-Torsion Fields}
\label{sec:repulsive}
...
\begin{definition}[Anti-Torsion Field~\cite{atfield}]
An Anti-Torsion Field (AT-field) is a region $\mathcal{R}$ where $J(x)<0$. Dual-sector rotation dominates, yielding effective repulsion. AT-fields are engineerable in principle via coherent excitation of $\theta_a$.
\end{definition}

For warp entry, an AT-field is not merely a barrier but the \emph{operational medium}: the vessel transitions into a locally generated or cosmically encountered $J<0$ shell, coasts with reduced gravitational coupling, and exits at a scheduled phase boundary. Stability of AT-fields under perturbation follows from self-reinforcing mismatch dynamics~\cite{atfield}: intrusion of external matter increases $|\beta_a|$, driving $J$ more negative.
...
\end{equation}
Active feedback via the same THz cavity damps unwanted excitations. AT-field self-reinforcement~\cite{atfield} provides additional stability against external perturbation.
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item \textbf{AT-field generation.} High-intensity circularly polarised THz ($10^{12}$--$10^{15}\,\mathrm{Hz}$) to drive $J<0$ locally; measure repulsive deflection or weight reduction.
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item Repulsive layers ($J<0$) and AT-fields provide the operational medium for effective superluminal coasting.
...
\bibitem{atfield}
Anonymous,
``Anti-Torsion Field: Engineerable Gravitational Repulsion in Dual Spacetime Theory,''
2025.
