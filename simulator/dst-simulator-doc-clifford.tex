...
\begin{gather}
  ...
  iK=Ki,~jK=Kj,~kK=Kk
\end{gather}
これらを組み合わせて2つの四元数の直積を作りますが、利便性の為に以後「双四元数」(バイクォータニオン)と呼称します。
\begin{equation}
  \begin{split}
    \texttt{Biuaternion}:\mathbb{H}\otimes\mathbb{H}\coloneq ~&a+bi+cj+dk+m+ni+rj+sk\\
    &a,b,c,d\in\mathbb{R}\\
...
\begin{gather}
  \bm{d} = d_1I+d_2J+d_3K,\quad d_1^2+d_2^2+d_3^2=1,\quad d_1,d_2,d_3\in\mathbb{R}
\end{gather}
で与えられる時、
\begin{equation}
  \begin{split}
    R = ~ & \exp \left( \frac{1}{2} \theta \bm{d} i \right) \\
    = ~ & \cosh \frac{\theta}{2} + \bm{d} i \sinh \frac{\theta}{2} \\
