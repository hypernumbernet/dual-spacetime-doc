...
\end{equation}
と定義できます。この距離は、$R_1^\dagger R_2 = \exp(\theta \hat{n})$（$\hat{n}$は回転軸の単位ベクトルに対応するbivector）の場合に$d(R_1, R_2) = \theta$となります。

簡単な例として$R_1 = 1$, $R_2 = \exp(\theta \hat{v})$（$\hat{v}^2 = -1$, $\|\theta \hat{v}\| = \theta$）を考えます。
このとき、
\begin{equation}
  \begin{split}
    \mathrm{Slerp}(1, \exp(\theta \hat{v}); t) 
    &= \exp(t \theta \hat{v}) \\
    &= \cos(t\theta) + \hat{v} \sin(t\theta), \quad t \in [0,1]
  \end{split}
\end{equation}
また、
\begin{equation}
  B(\log(R_2), \log(R_2)) = B(\theta \hat{v}, \theta \hat{v}) = -\theta^2
\end{equation}
（正規化によっては係数が異なる場合があります）となり、
\begin{equation}
  d(1, R_2) = \sqrt{-B(\log(R_2), \log(R_2))} = \theta
\end{equation}
...
このとき、方向$\hat{d}$へのラピディティ$\theta$のブーストローターは
\begin{equation}
  R = \exp(\theta \hat{d} \varepsilon) = \cosh\theta + \hat{d} \varepsilon \sinh\theta
\end{equation}
と書けます。
...
よって指数写像は
\begin{equation}
  \exp(\theta P) = \sum_{n=0}^\infty \frac{(\theta P)^n}{n!} = \cosh\theta + P \sinh\theta
\end{equation}
となり、成分では時間成分が$\cosh\theta$、空間成分が$d_i \sinh\theta$となります。

いま、単位ローター$R_1 = 1$、純粋ブースト$R_2 = \exp(\theta \hat{d} \varepsilon) = \cosh\theta + \hat{d} \varepsilon \sinh\theta$を考えます。
この間の対数線形補間（Log-linear interpolation）は
\begin{equation}
  \mathrm{Slerp}(1, R_2; t) = \exp\left( t \log(R_2) \right) = \exp(t \theta \hat{d} \varepsilon)
  = \cosh(t\theta) + \hat{d} \varepsilon \sinh(t\theta), \quad t \in [0,1]
\end{equation}
です。これはラピディティを$t\theta$に線形補間したブーストに対応し、リー代数的には単純実数三次元空間と同相です。
...
純粋ブースト$X = \theta \hat{d} \cdot \boldsymbol{P}$に対し、
\begin{equation}
  B(X, X) = \theta^2 B(\hat{d} \cdot \boldsymbol{P}, \hat{d} \cdot \boldsymbol{P}) = \theta^2 \cdot (+4) = +4\theta^2
\end{equation}
...
そこで$R_1 = 1$, $R_2 = \exp(\theta \hat{d} \varepsilon)$間のブースト強度（ラピディティ）を
\begin{equation}
  d(1, R_2) = \sqrt{B(\log R_2, \log R_2)} = \sqrt{4} \theta = 2\theta
\end{equation}
と定義できます。
